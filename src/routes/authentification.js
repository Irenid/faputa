...
module.exports = async function routes (fastify, options) {
  fastify.post("/signup", { schema: registerSchema }, async (req, res) => {
    ...
    try {
      ...
      res.header("HX-Redirect", "/users/signin");
      return "";
    } catch (e) {
      // Если Prisma выдает ошибку уникальности (P2002)
      if (e.code === 'P2002') {
        return `<div class="alert alert-danger m-0">Логин или email уже заняты</div>`;
      }
      return `<div class="alert alert-danger m-0">Неверные данные при регистрации</div>`;
    }
  });
  ...
  fastify.post("/signin", {schema: signinSchema}, async (req, res) => {
    ...
    if (!user || !(await bcrypt.compare(password, user.password))) {
      return `<div class="alert alert-danger m-0">Неверный логин или пароль</div>`;
    }
